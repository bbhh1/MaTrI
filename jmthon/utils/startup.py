...
from jmthon.helper import inline_mention#, check_update
...
async def group_ub():
    ...
    if not log_chat:
        ...
        try:
            group_id = await jmubot(
                CreateChannelRequest(
                    title="مجموعة الحفظ لماتركس",
                    about="مجموعة حفظ الأحداث والتنبيهات الخاصة بسورس ماتركس\n\n انضم لقناة السورس @VV333K",
                    megagroup=True,
                ),
            )
    ...
    if isinstance(chat.photo, ChatPhotoEmpty):
        pic = await jmubot.upload_file(file="resources/Jmthon.jpg")
        try:
