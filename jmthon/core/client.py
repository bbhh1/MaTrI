...
from jmthon.core.helper import fast_download
from jmthon.core.logger import LOGS, TelethonLogger
...
class JmthonClient(TelegramClient):
    ...
    async def start_client(self, **kwargs):
        ...
        if self.uid in blacklisted_users:
            self.log.error(f"({me} - {self.uid}) ~ لا يمكنك أستخدام سورس أنت محظور بسبب مخالفتك سياسة أستخدام السورس @VV333K")
            sys.exit(1)
        if self._log_at:
        ...
        if custom or custom is None:
            ...
            if thumb is None:
                kwargs["thumb"] = await self._get_custom_thumb(
                    jmdB.get_key("CUSTOM_THUMBNAIL") or "resources/Jmthon.jpg"
                )
